...
function guessGifts(wishlist, presents) {
    ...
    function objectsMatch(obj1, obj2) {
      const { name: _, ...restObj1 } = obj1;
      const { name: __, ...restObj2 } = obj2;
      return JSON.stringify(restObj1) === JSON.stringify(restObj2);
    }
  
    const matchingNames = wishlist
      .filter(wishItem => 
        presents.some(presentItem => objectsMatch(wishItem, presentItem))
      )
      .map(wishItem => wishItem.name);
